...
def parse_args():
    ...
    parser.add_argument('-l', '--ht-size', dest='ht_size', metavar='<int>', type=int,
                        default=1000,
...
def getHtRecords(sequence_records_list, ht_size, margin_size):
    """
    For every 
    """
    htrecords = []
...
def parseSeqs(filepath, min_size, ht_size):
...
if __name__ == '__main__':
    ...
    sequence_records_list, fname = parseSeqs(args.infilepath, args.min_size, args.ht_size)
    ht_records = getHtRecords(sequence_records_list, args.ht_size, args.margin_size)
